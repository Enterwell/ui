import { Button, Grid, Popover, TextField, type Theme, useMediaQuery } from "@mui/material";
...
function durationGetTime(duration: Duration) {
  return (
    (typeof duration.years !== "undefined" ? duration.years * 365 * 24 * 60 * 60 * 1000 : 0) +
...
export function DateTimeRangePicker({
  ...
  const combineDateAndTime = () => {
    if (dateValue[0] == null || dateValue[1] == null) return [];

    let startDurationTime = durationGetTime(
      intervalToDuration({
        start: startOfDay(new Date()),
        end: parse(startTime, "HH:mm", new Date()),
      }),
    );
    const endDurationTime = durationGetTime(
      intervalToDuration({
        start: startOfDay(new Date()),
  ...
  const preselectedOptionsOrDefault: { name: string; startDate: Date; endDate: Date }[] =
    useMemo(() => {
      if (preselectOptions) return preselectOptions;
      ...
      return [
        ...
        {
          name: "Proteklih 7 dana",
          startDate: sub(startOfDay(new Date()), { days: 7 }),
          endDate: endOfDay(new Date()),
        },
        {
          name: "Proteklih 30 dana",
          startDate: sub(startOfDay(new Date()), { days: 30 }),
          endDate: endOfDay(new Date()),
        },
        { name: "Ovaj mjesec", startDate: startOfMonth(new Date()), endDate: endOfDay(new Date()) },
        {
          ...
          endDate: endOfMonth(sub(startOfMonth(new Date()), { months: 1 })),
        },
      ];
    }, [preselectOptions]);
